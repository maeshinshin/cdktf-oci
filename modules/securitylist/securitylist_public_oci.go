...
func NewPublicSecurityList(stack constructs.Construct, compartmentId *string, vcnId *string, options ...Option) *securitylist {
	config := &Config{
		DisplayName: defaultPublicSecurityListName,
		Config: util.Config{
			FreeformTags: util.DefaultTags,
		},
		EgressSecurityRules:  defaultEgressSecurityRules,
		IngressSecurityRules: defaultPublicIngressSecurityRules,
	}
	...
	securitylist := &securitylist{coresecuritylist.NewCoreSecurityList(stack, jsii.String("securitylist-public-cdktf-oci"), securitylistConfig)}
