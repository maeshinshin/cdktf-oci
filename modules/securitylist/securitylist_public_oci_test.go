...
var propaties_public1 = &map[string]any{
...
var (
	propaties_public2 = &map[string]any{}
	propaties_public3 = &map[string]any{}
	propaties_public4 = &map[string]any{}
)

func TestNewPublicSecurityListWithCorrectpropaties_public(t *testing.T) {
	for k, v := range *propaties_public1 {
		(*propaties_public2)[k] = v
		(*propaties_public3)[k] = v
	}
	(*propaties_public2)["DisplayName"] = "test"
	(*propaties_public3)["IngressSecurityRules"] = append((*propaties_public3)["IngressSecurityRules"].([]map[string]any), map[string]any{
		"Description": "Allow telnet traffic from the internet",
		"Stateless":   false,
		"Source":      "0.0.0.0/0",
		"SourceType":  "CIDR_BLOCK",
		"Protocol":    "6",
	...
	for k, v := range *propaties_public3 {
		(*propaties_public4)[k] = v
	}
	(*propaties_public4)["DisplayName"] = "test"
	...
	test := []struct {
		...
		{
			name:       "no options",
			properties: propaties_public1,
		},
		{
			name:       "with Display Name",
			properties: propaties_public2,
			options: []Option{
		...
		},
		{
			...
			properties: propaties_public3,
			options: []Option{
				AddIngressSecurityRule(coresecuritylist.CoreSecurityListIngressSecurityRules{
					Description: jsii.String("Allow telnet traffic from the internet"),
	...
	for _, tt := range test {
		t.Run(tt.name, func(t *testing.T) {
			...
			NewPublicSecurityList(stack, compartment.Id(), vcn.Id(), tt.options...)
